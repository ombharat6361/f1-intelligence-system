...
OpenF1Client is mocked so these run offline without API credentials.
"""
...
@pytest.fixture
def mock_client():
    with patch("src.agents.data_agent.OpenF1Client") as MockClass:
...
async def test_data_node_calls_all_four_client_methods(mock_client):
    await data_node(BASE_STATE)
    mock_client.get_race_results.assert_called_once_with(BASE_STATE["race_id"])
    mock_client.get_lap_times.assert_called_once_with(BASE_STATE["race_id"])
    mock_client.get_pit_stops.assert_called_once_with(BASE_STATE["race_id"])
    mock_client.get_sector_times.assert_called_once_with(BASE_STATE["race_id"])
